...
public async Task<byte[]> ExportTeamRankingToExcelAsync(ClaimsPrincipal evaluatorClaims, int evaluationPeriodId)
    {
        var rankings = await _reportService.GetTeamRankingAsync(evaluatorClaims, evaluationPeriodId);

        using var workbook = new XLWorkbook();
        var worksheet = workbook.Worksheets.Add("Ekip Sıralaması");
