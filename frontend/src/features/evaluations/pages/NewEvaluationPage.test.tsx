...
// NOT: Bu sayfa 4 ayrı API modülünü paralel çağırıyor
// (evaluationPeriodsApi, evaluatorEmployeesApi, criteriaApi, evaluationsApi).
// Hepsi burada mock'lanıyor; "Tahmini Toplam Skor" kartındaki
// ağırlıklı ortalama hesaplaması (kategori bazlı ortalama * kategori
// ağırlığı, kategoriler arası toplam) doğrudan bu testlerle doğrulanıyor.
...
const categories = [
    { id: 1, name: 'Teknik Yetkinlik', weight: 60, isActive: true },
    { id: 2, name: 'İletişim', weight: 40, isActive: true },
]
...
async function selectEmployee(
    ...
    const combobox = screen.getByRole('combobox', { name: /çalışan/i })
...
describe('NewEvaluationPage', () => {
    ...
    it('ekipte çalışan yoksa uygun mesajı göstermeli', async () => {
        ...
        expect(
            await screen.findByText('Ekibinde çalışan bulunmuyor')
        ).toBeInTheDocument()
    })
    ...
    it('çalışan seçilince kriterler kategorilere göre gruplanıp gösterilmeli', async () => {
        ...
        expect(await screen.findByText('Kod Kalitesi')).toBeInTheDocument()
        expect(screen.getByText('Problem Çözme')).toBeInTheDocument()
        expect(screen.getByText('Takım İçi İletişim')).toBeInTheDocument()
    ...
    it('tüm kriterler puanlanmadan gönder butonu devre dışı olmalı', async () => {
        ...
        expect(
            screen.getByRole('button', { name: /değerlendirmeyi gönder/i })
        ).toBeDisabled()
        expect(
            screen.getByText(/tüm kriterleri puanlamalısın/i)
        ).toBeInTheDocument()
    })
    ...
    it('ağırlıklı tahmini toplam skoru doğru hesaplamalı', async () => {
        ...
        // Teknik Yetkinlik (%60): Kod Kalitesi=4, Problem Çözme=5 -> ort. 4.5
        // İletişim (%40): Takım İçi İletişim=3 -> ort. 3
        // Ağırlıklı toplam = 4.5*0.6 + 3*0.4 = 2.7 + 1.2 = 3.9
        //
        // Kriterler DOM'da sırasıyla render edildiği için (Kod Kalitesi,
        // Problem Çözme, Takım İçi İletişim), her puan değerinin kaçıncı
        // ScoreSelector'a ait olduğunu index ile seçiyoruz.
        await user.click(
            screen.getAllByRole('button', { name: '4 puan' })[0]
        )
        await user.click(
            screen.getAllByRole('button', { name: '5 puan' })[1]
        )
        await user.click(
            screen.getAllByRole('button', { name: '3 puan' })[2]
        )

        expect(await screen.findByText('3.90 / 5')).toBeInTheDocument()
    ...
    it('gönder butonuna basılınca createEvaluation doğru payload ile çağrılmalı', async () => {
        ...
        const scoreButtons = screen.getAllByRole('button', { name: '4 puan' })
        // Her kriter için ayrı ScoreSelector render edildiğinden 3 tane "4 puan" butonu var.
        ...
        await waitFor(() => {
            expect(mockedCreateEvaluation).toHaveBeenCalledTimes(1)
        })

        const payload = mockedCreateEvaluation.mock.calls[0][0]
        expect(payload.employeeId).toBe(EMPLOYEE_ID)
        expect(payload.evaluationPeriodId).toBe(activePeriod.id)
        expect(payload.comment).toBe('Genel olarak başarılı bir dönem.')
        expect(payload.scores).toEqual(
            expect.arrayContaining([
                { performanceCriterionId: 1, score: 4 },
                { performanceCriterionId: 2, score: 4 },
                { performanceCriterionId: 3, score: 4 },
            ])
        )
    })
    ...
    it('başarılı gönderim sonrası başarı mesajı göstermeli ve formu sıfırlamalı', async () => {
        ...
        expect(
            await screen.findByText('Değerlendirme başarıyla kaydedildi.')
        ).toBeInTheDocument()
    })

    it('API hatası durumunda sunucudan gelen mesajı göstermeli', async () => {
        mockedCreateEvaluation.mockRejectedValueOnce({
            response: { data: { message: 'Bu dönem için zaten değerlendirilmiş.' } },
        })
        ...
        expect(
            await screen.findByText('Bu dönem için zaten değerlendirilmiş.')
        ).toBeInTheDocument()
    })
    ...
    it('bu dönemde zaten değerlendirilmiş çalışan seçenek listesinde tamamlandı olarak işaretlenmeli', async () => {
        ...
        expect(
            within(listbox).getByText('✓ Tamamlandı')
        ).toBeInTheDocument()
    })
})